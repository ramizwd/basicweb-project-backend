...
const getCommentVote = async (userId, commentId, next) => {
    try {
        const [rows] = await promisePool.execute(
            'SELECT user_id, vote_count, comments_id FROM pjr_comments_vote WHERE user_id = ? AND comments_id = ?',
            [userId, commentId]
        );
        return rows[0];
    } catch (e) {
...
const commentVoteInsert = async (user, commentVote, commentId, next) => {
    try {
        const [rows] = await promisePool.execute(
            'INSERT INTO pjr_comments_vote (user_id, vote_count, comments_id) VALUES (?,?,?)',
            [user.user_id, commentVote.vote_count, commentId]
        );
...
const deleteCommentVote = async (bodyInfo, commentId, next) => {
    try {
        const [rows] = await promisePool.execute(
            'DELETE FROM pjr_comments_vote WHERE user_id = ? AND vote_count = ? AND comments_id = ?',
            [bodyInfo.user_id, bodyInfo.vote_count, commentId]
        );
...
const commentVoteUpdate = async (commentVote, commentId, next) => {
    try {
        const [rows] = await promisePool.execute(
            'UPDATE pjr_comments_vote SET vote_count = ? WHERE user_id = ? AND comments_id = ?',
            [commentVote.vote_count, commentVote.user_id, commentId]
        );
